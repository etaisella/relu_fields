...
def accumulate_radiance_density_on_rays_va(
    processed_points: ProcessedPointsOnRays,
    id_rays: Tensor,
    rays: Rays,
    stochastic_density_noise_std: float = 1.0,
    ...
    # get map of contributing voxels
    contributing_voxel_index = torch.argmax(weights, dim=-1)
    ray_indices = torch.arange(id_rays.shape[0])
    contributing_voxel_map = id_rays[ray_indices[:], contributing_voxel_index[:]]

    # filter out locations where no voxels affect the image (background)
    background_mask = torch.sum(raw_density, dim=-1) == 0
    ...
    render_out_result = RenderOut(
        colour=colour_render,
        depth=depth_render,
        extra=extra_dict,
    )

    return render_out_result, contributing_voxel_map
